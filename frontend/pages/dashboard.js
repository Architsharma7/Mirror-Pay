      ...
      return (
        <div className="flex bg-white justify-between flex-col">
          <div
            className={`${
              open ? "w-72" : "w-20"
            } duration-300 h-screen bg-black relative p-4 pt-8`}
          >
            <img
              src="../images/control.png"
              alt=""
              className={`absolute cursor-pointer -right-4 top-9 w-9 border-2 border-black rounded-full ${
                !open && "rotate-180"
              }`}
            ...
            <div className="flex items-center gap-x-4">
              ...
              />
              <h1
                className={`text-white origin-left ml-2 font-medium text-xl duration-300 ${
                  !open && "scale-0"
                }`}
              >
                MirrorPay
            ...
            <ul className={`text-white pt-6`}>
              {Menus.map((menu, index) => (
                <li
                  ...
                  className={`${
                    menu.gap ? "mt-9" : "mt-2"
                  } text-sm flex items-center gap-x-4 cursor-pointer p-2 rounded-md hover:bg-slate-400`}
                >
                  <img src={`../images/${menu.src}.png`} />
                  <span
                    className={`${!open && "hidden"} origin-left duration-300`}
                  >
                    <a onClick={() => router.push(`./${menu.title}`)}>
                      {menu.title}
                    </a>
                  </span>
                </li>
